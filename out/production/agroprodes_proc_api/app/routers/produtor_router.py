...
from app.services.validacoes.validacao_zarc_service import ValidacaoZarcService, PlanejamentoAgronomicoDTO
...
# ==============================================================================
@router.post("/validar-zarc", status_code=status.HTTP_200_OK)
async def validar_planejamento_agroclimatico(
        dados: PlanejamentoAgronomicoDTO,
        db_principal: AsyncSession = Depends(get_async_db)
):
    """
    Valida se a Data de Plantio e Colheita fornecidas no formulário estão em conformidade
    com as regras de decêndio e teto de risco admissível da tabela agroprods.zarc_zoneamento.
    """
    service = ValidacaoZarcService(db_principal)
    return await service.validar_planejamento_zarc(dados)
